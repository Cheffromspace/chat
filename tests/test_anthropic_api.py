...
class TestInvokeAnthropicAPI(unittest.TestCase):
    def setUp(self):
...
def test_successful_api_call(self, mock_client):
    ...
    self.assertIsNotNone(result)  # Add this line to check if result is not None
    self.assertIn("Paris is the largest city and capital of France.", result)
...
def test_bad_request_error(self, mock_client):
    ...
    mock_client.return_value.messages.create.side_effect = anthropic.BadRequestError(
...
def test_rate_limit_error_with_retry(self, mock_client):
    ...
    self.assertIn("Paris is a beautiful city with a rich cultural heritage.", result)
    self.assertTrue(
        any(
